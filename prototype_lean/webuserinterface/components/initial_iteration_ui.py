...
class InitialIterationUI(UIComponent):
    def build_userinterface(self):
        with ngUI.column().classes('mx-auto items-center') \
                .bind_visibility_from(self.webUI, 'is_initial_iteration', value=True):

            with ngUI.column().classes('items-center') \
                    .bind_visibility_from(
                        self.webUI, 'study_phase_is_task',
                        backward=lambda is_task: not is_task
                    ):
                ngUI.markdown("""
**Interactive Demo**
...
- `an illustration of love`
            ...
            with ngUI.card().classes('self-center no-box-shadow bg-grey-3 p-0 m-0 mt-4 gap-0') \
                    .style('border-radius: 30px;').tight():
                with ngUI.column().classes('items-stretch p-0 gap-0'):
                    ...
                    with self.prompt_field.add_slot("append"):
                        ngUI.button(icon='start', on_click=self.on_generate_images_button_click) \
                            .props('flat fab color=black').tooltip('Generate images')
    ...
    async def on_generate_images_button_click(self):
        ...
        # Reset image selection whenever a fresh batch is generated
        if hasattr(self.webUI, 'study_selected_image_idx'):
