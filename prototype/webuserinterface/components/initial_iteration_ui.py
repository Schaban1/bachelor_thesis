        ...
        with ngUI.column().classes('mx-auto items-center').bind_visibility_from(self.webUI, 'is_initial_iteration', value=True):
            ...
            with ngUI.card() \
                ...
                with ngUI.column().classes('items-stretch p-0 gap-0'):
                    ...
                    self.recommendation_field = ngUI.select({t: t.value for t in [RecommendationType.BASELINE, RecommendationType.SIMPLE, RecommendationType.SIMPLE2, RecommendationType.RANDOM, RecommendationType.EMA_DIRICHLET]}) \
