        ...
        with self:
            with ngUI.element('div').style("margin-left: 8px;").classes("h-screen flex"):
                with ngUI.column().classes('p-0 gap-0'):
                    ...
                    ngUI.select({m.value: m.value for m in ScoreMode}, label='score_mode', on_change=self.webUI.reload_userinterface).props(self.input_props).bind_value(self.webUI, 'score_mode')
