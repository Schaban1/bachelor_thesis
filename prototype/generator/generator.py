...
class GeneratorStream(GeneratorBase):
    ...
    def __init__(self,
        ...
        self.stream.vae = AutoencoderTiny.from_pretrained("madebyollin/taesd", torch_dtype=self.pipe.dtype).to(
    ...
    def load_generator(self):
        ...
        if self.use_negative_prompt:
            ...
            negative_prompt_tokens = self.pipe.tokenizer(self.negative_prompt,
                                                         ...
                                                         truncation=True,
                                                         return_tensors="pt", ).to(self.pipe.text_encoder.device)
            self.negative_prompt_embeds = self.pipe.text_encoder(negative_prompt_tokens.input_ids)[0].repeat(
                self.n_images, 1, 1)
